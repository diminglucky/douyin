    ...
    .then((res) => {
      if (res.code === 0) {
        ...
        if (!res.data.downImageVideoFromEnv) {
          columns.value = [
            { title: '名称', dataIndex: 'userName', width: 100 },
            { title: '状态', dataIndex: 'status', width: 100 },
            { title: '收藏路径', dataIndex: 'savePath', width: 150, ellipsis: true },
            { title: '喜欢路径', dataIndex: 'favSavePath', width: 150, ellipsis: true },
            { title: '博主路径', dataIndex: 'upSavePath', width: 150, ellipsis: true },
            { title: 'Cookie', dataIndex: 'cookies', width: 80 },
            { title: '有效状态', dataIndex: 'statusMsg', width: 100 },
            { title: '操作', dataIndex: 'edit', width: 140 },
          ];
        } else {
          columns.value = [
            { title: '名称', dataIndex: 'userName', width: 100 },
            { title: '状态', dataIndex: 'status', width: 100 },
            { title: '收藏路径', dataIndex: 'savePath', width: 140, ellipsis: true },
            { title: '喜欢路径', dataIndex: 'favSavePath', width: 140, ellipsis: true },
            { title: '博主路径', dataIndex: 'upSavePath', width: 140, ellipsis: true },
            { title: '图文路径', dataIndex: 'imgSavePath', width: 140, ellipsis: true },
            { title: 'Cookie', dataIndex: 'cookies', width: 80 },
            { title: '有效状态', dataIndex: 'statusMsg', width: 100 },
            { title: '操作', dataIndex: 'edit', width: 140 },
          ];
  ...
  <a-table v-bind="$attrs" :columns="columns" :dataSource="dataSource" :pagination="false">
    ...
    <template #bodyCell="{ column, text, record }">
      ...
      <template v-else-if="column.dataIndex === 'edit'">
        <a-button :disabled="showModal" type="link" @click="edit(record)" size="small">
          <template #icon>
            <EditFilled />
          </template>
          编辑
        </a-button>
        <a-button type="link" @click="deleted(record.id)" danger size="small">
          <template #icon>
            <DeleteFilled />
          </template>
          删除
      ...
      <template v-else-if="['savePath', 'favSavePath', 'upSavePath', 'imgSavePath'].includes(column.dataIndex)">
        <a-tooltip :title="text" placement="topLeft">
          <span class="path-cell">{{ text }}</span>
        </a-tooltip>
      </template>
